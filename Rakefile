...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/hfpp2012copy/tinymce_extended/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("tinymce_extended.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in tinymce_extended.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "tinymce_extended.gemspec", /ruby_version = .*">= (.*)"/ => lowest
